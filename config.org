...
** WAIT Convert hugo tags to org-roam tags

#+begin_src emacs-lisp :tangle no
(defun gps-duplicate-current-line ()
    "Duplicates the current line below."
    (let ((current-line (thing-at-point 'line)))
      (insert "\n")
      (insert current-line)))

(defun gps-duplicate-hugo-tags-as-roam-tags ()
  "Duplicate the hugo_tags as roam_tags to export them with ox-hugo."
  (let ((current-point (point)))
    (goto-char (point-min))
    (if (re-search-forward "^#\\+ROAM_TAGS.+$" nil t)
        (goto-char point)
      (let ((hugo_tag (re-search-forward "^#\\+HUGO_TAGS.+$" nil t)))
        (if hugo_tag
            (progn (goto-char hugo_tag)
                   (gps-duplicate-current-line )
                   (re-search-backward "^#\\+HUGO_TAGS:" nil t)
                   (replace-match "#+ROAM_TAGS:")
                   (goto-char current-point)
                   (forward-line 2))
          (message "Regex did not match"))))))
(gps-duplicate-hugo-tags-as-roam-tags)

(defun gps-org-roam-mode-before-save-hook()
  "Before-save-hook for org-roam to add hugo_tags if roam_tags exist."
  (when (bound-and-true-p org-roam-mode)
        (gps-duplicate-hugo-tags-as-roam-tags)))
(add-hook 'before-save-hook 'gps-org-roam-mode-before-save-hook)
