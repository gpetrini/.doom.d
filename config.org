...
- org-gcal ships no save-triggered or timer-triggered sync, and none is added
  here: the sync is asynchronous and lock-guarded, so a save during a sync
  would fail on the lock, the post-sync save would retrigger it, and every
  half-typed heading would reach Google and its guests.
