...
* Org-todoist
:PROPERTIES:
:ID:       0f3a6c21-9b4e-4a7d-8c1f-5d2e6b07a913
:END:

Bidirectional sync of org headlines with Todoist via the sync API. Upstream
documentation is vendored in [[file:.scratch/research/README.org]].

The synced file *is* =org-gtd-tasks.org=. org-todoist re-serializes the whole
buffer on every sync and local changes overwrite remote changes.

Level 1 of that file is a contested namespace: org-gtd needs its six buckets at
level 1 (they carry =ORG_GTD_REFILE=), and org-todoist infers every unlabeled
level-1 headline as a Todoist project. The account is on the free tier, capped
at 5 projects, so exactly one bucket has to stay out. =Projects= is the one:

| Bucket        | Todoist  | Tasks |
|---------------+----------+-------|
| Actions       | project  |    14 |
| Someday/Maybe | project  |    39 |
| Habits        | project  |    22 |
| Projects      | =IGNORE= |     - |
| Tickler       | =IGNORE= |     - |
| Calendar      | =IGNORE= |     - |

Three of the five available slots are used. =Tickler= and =Calendar= are held
back: org-gtd declares =ORG_GTD_TIMESTAMP= required for both types
(=org-gtd-types.el=), org-todoist writes dates to =SCHEDULED=/=DEADLINE=, and
nothing reconciles the two. See issue #3.

=Projects= is excluded in place rather than moved to another file.
`org-gtd-default-file-name' is a defconst and `org-gtd-refile--get-gtd-targets'
only ever searches =org-gtd-tasks.org=, so a moved bucket makes
`org-gtd-refile--do' recreate an empty one here on the next project organize.

org-gtd stores ticklers, habits, appointments and someday items with no todo
keyword on the headline. org-todoist infers a keyword-less headline under a
project as a *section*, so without intervention those 87 items would sync as
empty Todoist sections. They carry an explicit =TODOIST_TYPE: TASK= instead,
which wins over the inference. The cost is accepted and deliberate: on the
first pull `org-todoist--set-todo' writes `org-todoist-todo-keyword' into every
task that lacks one, so 96 headlines gain =TODO=.

Habits are the weak point of this mapping. Their repeaters (=.+1d/2d=, =++1d=,
=REPEAT_CRON=) land in the recurring-task support the upstream author documents
as partial.

Further constraints found by reading the source, recorded here so they are not
rediscovered:

- The ignore property value is =IGNORE=, not =IGNORED= as the upstream README
  says; it must match `org-todoist--ignored-node-type'. A subtree has to be
  marked before its first sync -- `org-todoist-ignore-subtree' refuses subtrees
  already tracked by Todoist.
- A headline with no todo keyword sitting under a section is inferred as a task
  (`org-todoist--get-todoist-type'), and `org-todoist--set-todo' then writes
  `org-todoist-todo-keyword' into it on pull. Structural headings therefore
  acquire =TODO= unless their subtree is ignored.
- org-todoist only writes the =org-list-indent-offset= file header into files it
  creates, so an existing file needs it added by hand.

** API token

This repository is public, so the token is never written here. It is read from
=~/.authinfo= (which must be mode 600) through auth-source:

: machine api.todoist.com login <todoist-login> password <api-token>
