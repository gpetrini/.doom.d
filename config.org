...
- Dated tasks live in the org-gtd =Calendar= bucket and are never pushed to
  ...
  model.
- Every fetched event receives =ORG_GTD: Calendar= and =ORG_GTD_TIMESTAMP=
  through `org-gcal-after-update-entry-functions'. The Engage day block keeps
  only entries whose own =ORG_GTD= is =Calendar= or =Habit=, and the
  stuck-calendar review lists =Calendar= items lacking the timestamp. The
  properties survive later syncs because org-gcal only touches its own.
  Side effect: `org-gtd-archive-completed-items' moves cancelled (=KILL=)
  events to the yearly GTD archive instead of =gcal/<file>_archive=.
