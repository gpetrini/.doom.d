...
Google Calendar is mirrored by org-gcal into =~/Dropbox/GTD/gcal/=, one file per
calendar, each carrying its own =#+CATEGORY= and =#+FILETAGS=.
The subdirectory is added to `org-agenda-files' separately (the directory form
of that variable is not recursive).

Division of responsibility with org-gtd:

- Appointments (a fixed time agreed with other people) live in Google and in
  =gcal/=; =org-gcal-sync= is bidirectional inside =gcal/= only.
- Dated tasks live in the org-gtd =Calendar= bucket and are never pushed to
  Google. Their repeaters and state log have no counterpart in the Google event
  model.
- A fetched event is not an org-gtd item and carries no =ORG_GTD= property, so
  it is invisible to every type-filtered Engage block. Until 2026-09-22 each
  event was stamped =ORG_GTD: Calendar= plus =ORG_GTD_TIMESTAMP= to reach the
  day block; the stamp also made every past appointment a permanent entry of
  the "Missed events" block (=(type . calendar) (when . past)=), because a
  Google event has no TODO keyword and nothing ever marks it =DONE= or =KILL=,
  while the block only excludes done items. The day block is therefore a
  native agenda block whose skip function, `my/gtd-skip-unless-day-item',
  keeps org-gtd =Calendar= and =Habit= entries and anything carrying
  =org-gcal-managed=. Events reach it through the active timestamp in their
  =:org-gcal:= drawer, which `org-agenda-get-timestamps' scans like any other,
  not through =ORG_GTD_TIMESTAMP=.
  Side effect, unchanged: `my-gtd-archive-killed-items' matches on the
  keyword over all agenda files, so a cancelled (=KILL=) event goes to the
  yearly GTD archive instead of =gcal/<file>_archive=.
- A fetched event is never refiled into =org-gtd-tasks.org=. org-gcal appends
  new events as level-1 headings at the end of the target file, and its
  auto-archive uses `org-archive-location' from whatever file the event sits
  in, so an event refiled into the org-gtd file would end up in
  =org-gtd-tasks.org_archive=. When an event implies work, capture an org-gtd
  item with an =id:= link to the event: =, G c= on the event creates its =ID=
  and opens the inbox capture with the link as body.
- The reverse direction is the =g= capture template: it files a heading with
  an =:org-gcal:= drawer into the chosen =gcal/= file (the file decides the
  calendar, the drawer timestamp the time). org-gcal itself hooks
  `org-capture-after-finalize-hook' and `org-after-refile-insert-hook' to
  post any heading captured or refiled into a file of
  `org-gcal-fetch-file-alist', so the event is created when the capture is
  finalised. Those hooks only exist once org-gcal is loaded, which is why the
  template's target function requires it.
- org-gcal ships no save-triggered or timer-triggered sync. A save-triggered
  one is deliberately not added: the sync is asynchronous and lock-guarded, so
  a save during a sync would fail on the lock, the post-sync save would
  retrigger it, and every half-typed heading would reach Google and its
  guests. What runs instead is a fetch-only timer every ten minutes, started
  when org-gcal loads, that gives up rather than erroring when a sync is
  running, a =gcal/= buffer is modified or a conflict file exists. Pushes stay
  manual (=, G p=) and are ETag-guarded: a push over a stale copy gets HTTP
  412 and org-gcal then overwrites the local entry with the server version,
  never the reverse. Fetched entries carry =org-gcal-managed: gcal=, which
  bulk syncs never push. The residual risk is therefore an edit made in
  =gcal/= and never posted, not a conflict.
- Events cancelled in Google are marked =KILL=, the org-gtd cancellation
  keyword, and never removed; the auto-archive takes them once they leave the
  `org-gcal-up-days' window.
- Classroom calendars (=CE*=, =HO*=) and the contacts birthday calendar are not
  synced.

Constraints found by reading the source, recorded here so they are not
rediscovered:

- `org-gcal-sync' is asynchronous and does not save the buffers it writes to;
  the only `save-buffer' is on the auto-archive path, which runs before the
  fetch. Left alone, the =gcal/= buffers stay modified until something saves
  them, which is the main source of Syncthing conflicts. The advice below saves
  them when the deferred settles, and refuses to start a sync from a modified
  buffer or while a conflict copy (Syncthing =sync-conflict=, Dropbox
  "conflicted copy"/"conflito") exists. =, G x= lists such copies under the
  whole GTD directory; for =gcal/= files the rule is to delete the copy and
  fetch again, since Google holds the truth.
- org-gcal registers its OAuth provider at load time and warns if
  `org-gcal-client-id' or `org-gcal-client-secret' is nil at that point. The
  credentials are therefore set in `after!' =oauth2-auto=, which org-gcal
  requires before that check, so the =authinfo.gpg= file is only decrypted when
  org-gcal itself loads, not at startup.
- oauth2-auto keys its tokens by =(calendar-id . provider)=, so the first sync
  opens one browser authorisation per calendar. This happens once per machine.
- There is no interactive re-authorisation command. To start over, delete
  =~/.emacs.d/oauth2-auto.plist=.

*** Secrets

This repository is public, so nothing from the Google account is written here.
Everything is read from =~/.authinfo.gpg= through auth-source:

: machine org-gcal login <client-id> password <client-secret>
: machine gcal:personal        login gcal password <calendar-id>
: machine gcal:conferences     login gcal password <calendar-id>
: machine gcal:teaching        login gcal password <calendar-id>
: machine gcal:bureaucracy     login gcal password <calendar-id>
: machine gcal:research-groups login gcal password <calendar-id>
: machine gcal:supervisions    login gcal password <calendar-id>
: machine gcal:reviews         login gcal password <calendar-id>
: machine gcal:research        login gcal password <calendar-id>
: machine gcal:meetings        login gcal password <calendar-id>
: machine gcal:ysi             login gcal password <calendar-id>

The refresh token is stored by oauth2-auto in a plstore encrypted to the first
secret key in the GPG keyring, so `gpg-agent' caches the passphrase and Emacs
does not ask for a separate plstore password.
The whole block is gated on the existence of =~/.authinfo.gpg=, so Doom boots
unchanged on a machine that has not been set up.

*** Setting up a machine

1. Generate a key and fix the keyring permissions:
   : gpg --quick-gen-key "<name> <email>" future-default default never
   : chmod 700 ~/.gnupg
   =future-default= yields an ed25519 signing key plus a cv25519 encryption
   subkey. A bare =ed25519= key has no encryption capability and plstore fails
   with "Unusable public key"; repair one with
   =gpg --quick-add-key <key-id> cv25519 encr never=.
   On a second machine, import the existing key instead:
   : gpg --export-secret-keys --armor <key-id> > key.asc   # on the first machine
   : gpg --import key.asc && shred -u key.asc               # on the second
2. Google Cloud console: create a project, enable the Calendar API, create an
   OAuth client of type "Desktop app", copy the client ID and secret.
   On a second machine, reuse the same client.
3. Google Calendar, for each calendar: Settings -> Integrate calendar ->
   Calendar ID. The primary calendar's ID is the account e-mail.
4. Write the lines above into =~/.authinfo.gpg= (Emacs encrypts on save when
   the buffer is visited with =.gpg=). Copy the file as is to a second machine.
5. Run =org-gcal-fetch= and approve the browser prompt for each calendar.
6. Make the key passphrase a once-per-login event; every sync opens the
   plstore and otherwise asks again after gpg-agent's 10-minute default:
   : printf 'default-cache-ttl 34560000\nmax-cache-ttl 34560000\n' > ~/.gnupg/gpg-agent.conf
   : gpgconf --reload gpg-agent
   If =~/.authinfo.gpg= was encrypted symmetrically (Emacs falls back to that
   when no encryption-capable key exists), re-encrypt it to the key so only
   one passphrase remains: =(setq epa-file-encrypt-to '("<fingerprint>"))=,
   then save the file once.
