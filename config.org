...
- The reverse direction is the =g= capture template: it files a heading with
  an =:org-gcal:= drawer into the chosen =gcal/= file, and =, G p= on it
  creates the Google event (the file decides the calendar, the drawer
  timestamp the time). Posting stays a manual step; see below.
