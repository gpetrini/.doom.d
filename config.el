...
;; engrave-faces does not parse code: it loads the text in a buffer, activates the
;; major mode, runs font-lock and reads the faces. ESS is a third-party package that
;; Doom loads on demand, so without this require a batch export comes out monochrome.
(after! ox-latex
  (require 'ess-r-mode nil t))

;; ess-R-font-lock-keywords ships with six groups off. Without re-enabling these five,
;; only `<-' and comments get colour in the exported deck. F&T is left off on purpose.
(after! ess-r-mode
  (dolist (kw '(ess-fl-keyword:fun-calls
